...
import '@synthra-swap/v3-core/contracts/interfaces/callback/ISynthraV3FlashCallback.sol';
import '@synthra-swap/v3-core/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the Synthra V3 flash function
contract PairFlash is ISynthraV3FlashCallback, PeripheryPayments {
    ...
    function synthraV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external override {
    ...
    /// @notice Calls the pools flash function with data needed in `SynthraV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        ISynthraV3Pool pool = ISynthraV3Pool(PoolAddress.computeAddress(factory, poolKey));
